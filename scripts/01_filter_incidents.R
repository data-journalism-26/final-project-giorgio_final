# ─────────────────────────────────────────────────────────────────────────────
# 01_filter_incidents.R — Filter UNITED + IOM records to the storm window
#
# Purpose:
#   Slice the upstream UNITED "List of Refugee Deaths" and the IOM Missing
#   Migrants Project CSV down to the records that fall inside the Cyclone
#   Harry window (12 Jan – 17 Feb 2026, Central Mediterranean route).
#
# Inputs:
#   - UNITED  : upstream thesis RDS (path below — not redistributable)
#   - IOM     : data/iom_may_2026.csv (public MMP CSV download)
#
# Outputs:
...
#
# Usage:
#   Rscript scripts/01_filter_incidents.R
#
# Data availability:
#   The raw UNITED and IOM incident data are NOT redistributed with this
#   repository. The pre-filtered CSVs in data/ ship with the repo so the
#   rest of the pipeline is reproducible. The upstream data is available
#   from the original sources or upon request to the author.
# ─────────────────────────────────────────────────────────────────────────────
...
# To refresh with a newer MMP snapshot, drop the new CSV in `data/` and update
# IOM_CSV below. UNITED is rebuilt only when the upstream RDS is reachable.
