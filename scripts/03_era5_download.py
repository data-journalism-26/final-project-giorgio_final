"""
03_era5_download.py — ERA5 hourly single-levels for the Cyclone Harry window.

Purpose:
    Two sequential requests to the Copernicus CDS:
      1) 2026-01-12 .. 2026-01-31
      2) 2026-02-01 .. 2026-02-17

Variables : 10m u/v wind, MSLP, total precipitation, significant wave height
Bbox      : 45N -5E 28N 25E (Iberian formation through eastern Mediterranean)
Resolution: native ERA5 0.25 deg

Inputs:
    Requires a Copernicus CDS account + ~/.cdsapirc API token.

Outputs:
    data/era5/jan_atm_instant.nc
    data/era5/feb_atm_instant.nc

Usage:
    python3 scripts/03_era5_download.py jan        # January only
    python3 scripts/03_era5_download.py feb        # February only
    python3 scripts/03_era5_download.py both       # both halves (default)
"""
