"""Download MSG SEVIRI IR_108 over the Mediterranean for the storm window.

Loop: download one .nat product, immediately extract IR_108 cropped to the
Med as a ~5 MB NetCDF, delete the 271 MB .nat, move on. Peak transient
storage stays around one .nat at a time.

Window: 2026-01-12 to 2026-01-24 inclusive, 3-hourly cadence (104 frames).

Run:
  python3 scripts/06_download_msg.py
  python3 scripts/06_download_msg.py --dry-run     # list what would be fetched
  python3 scripts/06_download_msg.py --cadence 6   # 6-hourly instead of 3
"""
import argparse
import shutil
import signal
